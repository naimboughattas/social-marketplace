...
export default function AccountSettingsModal({
  ...
  const handleSubmit = async () => {
    ...
    if (formData.platform === "instagram") {
      window.location.href = `${import.meta.env.VITE_NEXT_PUBLIC_API_URL}/instagram/auth?userId=${user.id}`;
    }

    if (formData.platform === "youtube") {
      window.location.href = `${import.meta.env.VITE_NEXT_PUBLIC_API_URL}/youtube/auth?userId=${user.id}`;
    }

    if (formData.platform === "tiktok") {
      window.location.href = `${import.meta.env.VITE_NEXT_PUBLIC_API_URL}/tiktok/auth?userId=${user.id}`;
    }

    if (formData.platform === "x") {
      window.location.href = `${import.meta.env.VITE_NEXT_PUBLIC_API_URL}/x/auth?userId=${user.id}`;
    }

    if (formData.platform === "linkedin") {
      window.location.href = `${import.meta.env.VITE_NEXT_PUBLIC_API_URL}/linkedin/auth?userId=${user.id}`;
    }

    if (formData.platform === "facebook") {
      window.location.href = `${import.meta.env.VITE_NEXT_PUBLIC_API_URL}/facebook/auth?userId=${user.id}`;
    }
  };
