...
export default function ServiceModal({
  ...
  const [interactionType, setInteractionType] = useState<InteractionType>(
    service === "follow" ? "one-month" : "specific"
  );
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={onClose}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg p-6 w-full max-w-3xl">
              ...
              <Dialog.Title className="text-lg font-medium">
                {service === "follow"
                  ...
                  : service === "like"
                    ? "Obtenir un like de"
                    : service === "comment"
                      ? "Obtenir un commentaire de"
                      : "Obtenir un repost de"}{" "}
                {influencer.username}
              </Dialog.Title>
                    ...
                    ? [
                        {
                          ...
                          description:
                            "L'influenceur suivra le compte pendant 1 mois puis se désabonnera automatiquement",
                        },
                        {
                          id: "monthly",
                          title: "Renouveler le suivi tous les mois",
                          description:
                            "L'influenceur restera abonné et le montant sera automatiquement débité chaque mois",
                        },
                      ]
                    : [
                        {
                          ...
                          description:
                            "Sélectionnez un ou plusieurs posts existants",
                        },
                        {
                          id: "specific-future",
                          title: "Posts spécifiques + Futurs posts",
                          description:
                            "Sélectionnez des posts existants et recevez automatiquement des interactions sur vos futurs posts",
                        },
                        {
                          id: "future",
                          title: "Tous les futurs posts",
                          description:
                            "Recevez automatiquement des interactions sur tous vos nouveaux posts",
                        },
                      ]
                  ).map((type) => (
                    <RadioGroup.Item key={type.id} value={type.id} asChild>
                      <div
                        className={`relative flex items-start p-4 rounded-lg border-2 transition-colors cursor-pointer ${
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-baseline space-x-1">
                              <span className="font-medium text-purple-600">
                                {
                                  getPriceDisplay(type.id as InteractionType)
                                    .price
                                }
                              </span>
                              <span className="text-sm text-gray-500">
                                ...
                                <Tooltip.Root>
                                  ...
                                  <Tooltip.Portal>
                                    <Tooltip.Content
                                      ...
                                      sideOffset={5}
                                    >
                                      Une commande sera automatiquement passée
                                      dès qu'un nouveau post est détecté
                ...
                <div className="grid grid-cols-2 gap-2">
                  {accounts.map((account) => (
                    <AccountBanner
                      ...
                      onSelect={() => setTargetHandle(account.id)}
                    />
                  ))}
                </div>
              </div>
